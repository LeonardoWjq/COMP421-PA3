...
public class connectPG {
    ...
    public static void bookTickets() {
		...
		try {
			...
			String id = sarr[0];
			String method = sarr[1];
			int schedule = Integer.parseInt(sarr[2]);
			String[] info = show_schedules.get(schedule).split(";");
			...
			// Modify the table
			sql = String.format("SELECT sid FROM customer WHERE sid = '%1$s'", id);
			rs = stmt.executeQuery(sql);
			if (!rs.next()) {
				stmt.executeUpdate(String.format("INSERT INTO customer (sid) VALUES ('%1$s')", id));
			}
			
			rs = stmt.executeQuery("select max(payment_num) " + 
					"from payment ");
			rs.next();
			String payment_num = "" + Integer.parseInt(rs.getString("payment_num")) + 1;
			stmt.executeUpdate(String.format("INSERT INTO payment VALUE ('%1$s', '%2$s', %3$f, '%4$s', '%5$s')",
					payment_num, method, "" + (prices.get(schedule) * (sarr.length - 3)), id, "2020-04-14 12:01:00"));	//change

			rs = stmt.executeQuery("SELECT max(ticket_num) FROM tickets");
			rs.next();
			int ticket_num = Integer.parseInt(rs.getString("ticket_num")) + 1;
			
			for (int i = 3; i < sarr.length; i ++) {
				stmt.executeUpdate(String.format("INSERT INTO tickets (ticket_num, price, seat_num, addr, room_num, start_time, sdate, pid) VALUE ('%1$d', %2$d, %3$s, '%4$s', '%5$s')",
						ticket_num, prices.get(schedule) , sarr[i], info[0], info[1], info[2], info[3], payment_num));
				ticket_num ++;
			}
			*/
